...
case "$(printf "   settings\n勒   reload configs\nﮮ   updates\n嗀   set theme\n   set background\n   display settings\n   taskmanager\n   exit bspwm" | dmenu -l 20 -X 0 -Y 0 -W 660 -i -p "system:")" in
     "   settings")          fish ~/.config/scripts/dmenu/dm-settings.sh ;;
     "勒   reload configs")    fish ~/.config/scripts/dmenu/dm-reload-files.sh ;;
     ...
     "   exit bspwm")        fish ~/.config/scripts/dmenu/dm-sysstate.sh ;;
	*) exit 1 ;;
esac
